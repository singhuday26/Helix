"""
HELIX DEMO: Speculative Decoding vs Baseline Comparison

This script demonstrates the performance difference between:
1. Baseline Autoregressive Decoding (target model only)
2. Speculative Decoding (draft + target model)
...
def run_comparison():
    ...
    print(f"\n{Fore.CYAN}💡 Loading Helix Engine (this may take a moment on first run)...")
    print(f"{Fore.YELLOW}   Downloading models if needed... (~1GB){Style.RESET_ALL}")
    ...
    if baseline_results and speculative_results:
        ...
        print(f"   The draft model generates {avg_acceptance:.0%} correct tokens on average,")
        print(f"   reducing the number of slow target model calls.\n")
